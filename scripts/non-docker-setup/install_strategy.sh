#!/bin/bash
# Cryptobot Strategy Service Installation Script for Linux/macOS
...
CONFIG_DIR="/etc/cryptobot/strategy"
mkdir -p "$CONFIG_DIR"

# Check if config file exists from Phase 3
if [ -f "./config/strategy_service_config.json" ]; then
    cp "./config/strategy_service_config.json" "$CONFIG_DIR/config.json"
    ...
    cat > "$CONFIG_DIR/config.json" << EOF
{
    ...
    "strategy_paths": [
        "/opt/cryptobot/services/strategy/strategies"
    ],
    "backtest_service": {
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    log "Setting up systemd service for strategy service..."
    
    # Create systemd service file
    cat > /etc/systemd/system/cryptobot-strategy.service << EOF
[Unit]
Description=Cryptobot Strategy Service
After=network.target postgresql.service cryptobot-auth.service
...
EOF
    ...
    log "Strategy service has been installed and configured as a systemd service."
    log "To start the service, run: sudo systemctl start cryptobot-strategy.service"
elif [[ "$OSTYPE" == "darwin"* ]]; then
    # Create launchd plist for macOS
    log "Setting up launchd service for strategy service..."
    
    # Create launchd plist file
    cat > ~/Library/LaunchAgents/com.cryptobot.strategy.plist << EOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.cryptobot.strategy</string>
    ...
    <string>/tmp/cryptobot-strategy.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/cryptobot-strategy.err</string>
</dict>
</plist>
EOF

    log "Strategy service has been installed and configured as a launchd service."
    log "To start the service, run: launchctl load ~/Library/LaunchAgents/com.cryptobot.strategy.plist"
fi

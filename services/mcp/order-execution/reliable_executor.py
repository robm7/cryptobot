...
class ReliableOrderExecutor(OrderExecutionInterface):
    ...
    async def execute_order(self, order_params: Dict) -> Optional[str]:
        ...
        if not self._check_circuit():
            logger.warning("Circuit breaker open, rejecting order execution")
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        if not self._check_circuit():
            logger.warning("Circuit breaker open, rejecting order cancellation")
            return False
    ...
    async def get_order_status(self, order_id: str) -> Optional[Dict]:
        ...
        if not self._check_circuit():
            logger.warning("Circuit breaker open, rejecting status check")
            return None
